...
public class CalendarProgram{ 
	static JLabel currMonth, lblYear, currMonthBig, lblTodo;
	static JButton btnPrev, btnNext, btnPrevBig, btnNextBig, btnCurrent, btnSaveTodo;
	...
	static JPanel pnlCalendar, pnlCalendarBig, todoListPanel, allToDoListPanel;
	static int realYear, realMonth, realDay, currentYear, currentMonth;
	static Dimension screenSize;
	static JTextField txtTodo;
	...
	public static void main (String args[]){
		...
		//add controls - todo
		pane.add(todoListPanel);
//		todoListDatePickerPanel.add(datePickerPanel);
		todoListPanel.add(lblTodo);
		todoListPanel.add(txtTodo);
		todoListPanel.add(btnSaveTodo);
		pane.add(allToDoListPanel);
		...
		btnCurrent.setBounds(((screenSize.width - currMonth.WIDTH)/4) + 50, 70, 100, 25);
		stblCalendarBig.setBounds(20, 100, screenSize.width - 445, 500);  
		
		//set bounds for to do
		todoListPanel.setBounds(0, 340, 320, screenSize.height - 650);
		...
		allToDoListPanel.setBounds(0, 425, 320, screenSize.height - 475);
	...
	static class tblCalendarRenderer extends DefaultTableCellRenderer{
		public Component getTableCellRendererComponent (JTable table, Object value, boolean selected, boolean focused, int row, int column){
			...
			if (column == 0 || column == 6){ //Week-end
				setBackground(new Color(255, 220, 220)); //pink
			}
			else{ //Week
			...
			if (value != null){
				//if (Integer.parseInt(value.toString()) == realDay && currentMonth == realMonth && currentYear == realYear){ //Today
					setBackground(new Color(220, 220, 255)); //lilac
				//}
			}
			setBorder(null);
	...
	static class btnSaveTodo_Action implements ActionListener{
		public void actionPerformed (ActionEvent e){
			mtblCalendar.setValueAt(txtTodo.getText(), currentRow, currentCol);
			allToDoListPanel.add(new JLabel(txtTodo.getText()));
